...
void wxSFShapeCanvas::DrawBackground(wxDC& dc, bool WXUNUSED(fromPaint))
{
...
void wxSFShapeCanvas::DrawForeground(wxDC& WXUNUSED(dc), bool WXUNUSED(fromPaint))
{
	// do nothing here...
}
...
void wxSFShapeCanvas::OnRightUp(wxMouseEvent& WXUNUSED(event))
{
	// HINT: override it for custom actions...
...
void wxSFShapeCanvas::OnUpdateVirtualSize(wxRect& WXUNUSED(virtrct))
{
	// HINT: override it for custom actions...
}
...
void wxSFShapeCanvas::LoadCanvas(const wxStringInputStream& sstream)
{
     wxASSERT(m_pManager);
    if(!m_pManager)return;
    ...
    wxXmlDocument xmlDoc;
    xmlDoc.Load(*(wxInputStream*)&sstream);
...
void wxSFShapeCanvas::SaveCanvas(const wxStringOutputStream& sstream)
{
     wxASSERT(m_pManager);
    if(!m_pManager)return;
    ...
    wxXmlDocument xmlDoc;
    xmlDoc.SetRoot(root);
    //wxOutputStream* saveStream = (wxOutputStream*)&stream;
    xmlDoc.Save(*(wxOutputStream*)&sstream);
